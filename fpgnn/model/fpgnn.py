...
from fpgnn.data import create_graph, get_atom_features_dim
import csv
    ...
    def forward(self, smile):
        ...
        for i, one in enumerate(smile):
            ...
            if self.fp_type == 'mixed':
                    ...
                    cmf = AllChem.GetHashedMorganFingerprint(mol, 2, nBits=1024)
                    data = UIntSparseIntVect.GetNonzeroElements(cmf)
                    data = list(data.items())
                    desired_length = 1024
                    default_value = 0
                    filled_data = [default_value] * desired_length
                    for index, value in data:
                        filled_data[index] = value

                    fp = []
                    fp.extend(fp_phaErGfp)
                    fp.extend(filled_data)
            else:
                fp = []
        ...
        fp_tensor = torch.tensor(fp_list, dtype=torch.float32)

        if self.cuda:
            fp_tensor = fp_tensor.cuda()

        fpn_out = self.fc1(fp_tensor)
