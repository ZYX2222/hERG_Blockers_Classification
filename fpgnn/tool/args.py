...
def add_train_argument(p):
    ...
    p.add_argument('--metric',type=str,choices=['auc', 'apr', 'rmse','r2'],default=None,
...
def add_hyper_argument(p):
    p.add_argument('--search_num', type=int,default=20,
...
def set_train_argument():
    ...
    if args.metric is None:
        if args.dataset_type == 'classification':
            args.metric = ['aroc', 'apr']
        elif args.dataset_type == 'regression':
            args.metric = ['r2', 'rmse']

    
    if args.dataset_type == 'classification':
        valid_metrics = ['aroc', 'apr']
        if not all(metric in valid_metrics for metric in args.metric):
            raise ValueError('One or more metrics are invalid for classification.')

    if args.dataset_type == 'regression':
        valid_metrics = ['rmse', 'r2']
        if not all(metric in valid_metrics for metric in args.metric):
            raise ValueError('One or more metrics are invalid for regression.')

    if args.fp_type not in ['mixed', 'morgan']:
        raise ValueError('Fingerprint type is invalid.')
